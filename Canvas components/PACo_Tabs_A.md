...
A menu indicator (3 dots) is shown at the right when there are tabs to show which cannot be shown because the width is too small. The idea is that when the menu indicator is clicked on, a tabs menu is shown. This is also shown in the image above. This tabs menu is another PACo canvas component: PACo_TabsMenu_A
